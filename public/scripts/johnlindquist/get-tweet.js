...
let result = await page.evaluate(() => {
  ...
  ).filter((el) => el.firstChild.firstChild.firstChild.children[2])[0]
    .firstChild.firstChild.firstChild

  let nameDiv =
    tweetDiv.children[1].children[1].children[0].children[0].children[0]
  ...
  let image =
    tweetDiv.children[1].children[0].children[0].children[0].children[0]
      .children[0].children[1].children[0].children[1].src
  image = image.replace('_bigger', '_400x400')

  let article = tweetDiv.children[2]

  let isReply = article.children[0].innerText.startsWith('Replying to') ? 1 : 0
